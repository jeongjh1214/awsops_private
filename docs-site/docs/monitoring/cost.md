...
페이지 로드 시 Cost Explorer API 가용성을 자동 점검하고, 사용 불가 시 로컬 스냅샷으로 폴백합니다.

### 3단계 로딩 흐름
1. **Cost 가용성 체크** → `/awsops/api/steampipe?action=cost-check`
2. 가능하면 라이브 쿼리 실행 (Cost Explorer API → Steampipe)
3. 라이브 실패 시 → `/awsops/api/steampipe?action=cost-snapshot&accountId={id}` 호출하여 `data/cost/` 하위 마지막 캐시 로드
...
|------|------|
| 라이브 OK | 평시 화면 (배너 없음) |
| 스냅샷 폴백 | 노란 배너: `Last fetched: {snapshotDate}` |
| 전부 실패 | 빨간 배너: "No cached cost snapshots available. Visit this page when Cost Explorer is accessible to build a local cache." |

:::info MSP 자동 감지
Host 계정이 MSP(Managed Service Provider) 환경이면 Cost Explorer API가 비활성화돼 있을 수 있습니다 — `02-setup-nextjs.sh`가 설치 시점에 감지하여 `costEnabled: false`로 설정합니다. 이 경우 페이지는 즉시 스냅샷 폴백 또는 빈 상태로 진입합니다.
:::

:::tip 스냅샷 자동 빌드
라이브 쿼리가 성공할 때마다 자동으로 `data/cost/{accountId}.json`에 결과가 저장됩니다. MSP 환경이거나 일시적으로 권한이 빠진 경우에도 직전 데이터로 계속 조회할 수 있게 하는 안전장치입니다.
:::

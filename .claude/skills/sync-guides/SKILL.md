...
Canvas 컴포넌트 위치: `docs-site/src/components/diagrams/`
...
2. **기존 가이드 확인**: `docs-site/docs/{category}/{service}.md` 존재 여부
3. **가이드 생성/업데이트**:
   - Tier 1: Canvas React 컴포넌트 import + 아키텍처/배포 설명
   - Tier 2: Screenshot + StatsCard/차트/테이블 설명
   - 신규: 기존 가이드(예: `docs-site/docs/storage/ebs.md`)를 템플릿으로 참조
4. **영어 번역 생성**: `docs-site/i18n/en/docusaurus-plugin-content-docs/current/{category}/{service}.md`
...
**공통 훅**: `docs-site/src/components/diagrams/useCanvas.ts`
...
| src/app 경로 | sidebars.ts 카테고리 | docs-site/docs 경로 |
|-------------|---------------------|---------------|

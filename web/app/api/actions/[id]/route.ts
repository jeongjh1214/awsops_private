...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!UUID_RE.test((await params).id)) return NextResponse.json({ message: 'invalid plan id' }, { status: 400 });
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (op === 'cancel') {
    await getPool().query(`UPDATE action_plans SET status='canceled' WHERE plan_id=$1 AND status IN ('planned','approved')`, [(await params).id]);
    await recordAudit({ planId: (await params).id, phase: 'approve', principal: approver, decision: 'canceled' });
  ...
  if (process.env[flagName] !== 'true') {
    await recordAudit({ planId: (await params).id, phase: 'execute', principal: approver, decision: 'flag_off' });
  ...
  if (!(await killSwitchOn(ksParam))) {
    await recordAudit({ planId: (await params).id, phase: 'execute', principal: approver, decision: 'killswitch_blocked' });
  ...
  if (plan.created_by === approver) {
    await recordAudit({ planId: (await params).id, phase: 'execute', principal: approver, decision: 'denied_self_approval' });
  ...
  const ok = await setApprovedAndExecuting((await params).id, approver, jobId); // atomic 4-eyes + not-expired guard in SQL
  ...
  await getPool().query(
    ...
    [jobId, JSON.stringify({ rollback_plan: plan.rollback_plan, ...plan.dry_run, inputs }), (await params).id]);
  ...
  if (queueUrl) {
    ...
    await sqs.send(new SendMessageCommand({ QueueUrl: queueUrl, MessageBody: JSON.stringify({
      ...
      plan_id: (await params).id, payload: { rollback_plan: plan.rollback_plan, ...inputs }, dry_run: false }) }));
  }
  await recordAudit({ planId: (await params).id, jobId, actionName: plan.action_name, phase: 'execute', principal: approver, decision: 'approved' });

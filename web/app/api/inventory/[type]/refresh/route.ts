...
export async function POST(request: Request, { params }: { params: Promise<{ type: string }> }) {
  ...
  try {
    const sync = await triggerSync((await params).type); // warm Steampipe -> Aurora (seconds); 'busy' if locked
    const page = await readResources((await params).type, { limit: 100, offset: 0 });

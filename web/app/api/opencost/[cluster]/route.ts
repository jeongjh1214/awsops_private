...
export async function GET(request: Request, { params }: { params: Promise<{ cluster: string }> }) {
  ...
  if (!(await isClusterOnboarded((await params).cluster))) return json({ status: 'error', message: 'unknown cluster' }, 404);
  const config = await getOpencostConfig((await params).cluster);
  return json({ cluster: (await params).cluster, config }, 200);
}

// PUT — save config (admin only). Writes only the app's own Aurora (no cluster/AWS write).
export async function PUT(request: Request, { params }: { params: Promise<{ cluster: string }> }) {
  const user = await verifyUser(request.headers.get('cookie'));
  if (!user) return json({ status: 'error', message: 'unauthenticated' }, 401);

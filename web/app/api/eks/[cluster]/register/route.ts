...
export async function POST(request: Request, { params }: { params: Promise<{ cluster: string }> }) {
  ...
  if (!CLUSTER_NAME_RE.test(cluster)) return json({ status: 'error', message: 'invalid cluster name' }, 400);
  // Cluster must actually exist (spec §3.2 ①) — never emit a guide for arbitrary input.
  const known = (await listClusters()).some((c) => c.name === cluster);
  ...
  if (auth) {
    const ok = await setClusterAuth(cluster, user.sub, auth);
  ...
  const entry = await hasAccessEntry(cluster);
  if (entry !== true) {
    ...
    return json({ registered: false, cluster, access: entry === false ? 'no-entry' : 'unknown', guide: await onboardingGuide(cluster) }, 409);
  }
  // Terraform-managed clusters are already permanently allowed — idempotent no-op, no redundant DB row (P4 gate).
  if (isEnvCluster(cluster)) return json({ registered: true, managedBy: 'terraform' }, 200);
  const ok = await registerCluster(cluster, user.sub);
  if (!ok) return json({ status: 'error', message: 'registry storage unavailable' }, 503);
  return json({ registered: true }, 200);
}

export async function DELETE(request: Request, { params }: { params: Promise<{ cluster: string }> }) {
  const { cluster } = await params;
  const user = await verifyUser(request.headers.get('cookie'));
  if (!user) return json({ status: 'error', message: 'unauthenticated' }, 401);
  if (!(await isAdmin(user))) return json({ status: 'error', message: 'admin only' }, 403);
  ...
  const result = await unregisterCluster(cluster);

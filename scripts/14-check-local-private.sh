...
if [ -x node_modules/.bin/next ]; then
  ok "Next.js binary exists at node_modules/.bin/next"
else
  fail "node_modules/.bin/next is missing; run: npm ci"
fi
...
if command -v curl >/dev/null 2>&1; then
  next_code="$(curl -s -o /dev/null -w "%{http_code}" http://127.0.0.1:3000/awsops 2>/dev/null || true)"
  if [ "$next_code" = "200" ]; then
    ok "Next.js responds at http://127.0.0.1:3000/awsops"
  else
    warn "Next.js is not responding on /awsops (HTTP ${next_code:-000}); start with: npm run dev"
  fi

  config_code="$(curl -s -o /dev/null -w "%{http_code}" http://127.0.0.1:3000/awsops/api/steampipe?action=config 2>/dev/null || true)"
  if [ "$config_code" = "200" ]; then
    ok "Next.js config API responds"
  else
    warn "Next.js config API is not responding (HTTP ${config_code:-000})"
  fi
